        ...
        if ( (!clusters_usados.includes(b)) && sigInst.clusters[b].length == myvar[i] ) {
          sigInst.iterNodes(function(n) {
            if (n.id == sigInst.clusters[b][0]) {
              mytest.push(n.attr.attributes.country)
            }
          });
          if (mytest[0] !== "unknown") {
            a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;border:1px solid #fff;background:' + b + ';display:inline-block"></div>' + ' ' + mytest[0] + ' (' + sigInst.clusters[b].length + ' members)</a></div>');
          }
...
function Search(a) {
    ...
    this.search = function (a) {
        ...
        this.lastSearch = a;
        this.results.empty();
        if (2 >= a.length) this.results.html("<i>You must search for a name with a minimum of 3000 letters.</i>");
        else {
...
function nodeActive(a) {
    ...
    d = "";
		for (g in e) {
			...
			if (c.group != d) {
				d = c.group;
				f.push('<li class="cf" rel="' + '#FF0000' + '"><div class=""></div><div class="">' + d + "</div></li>");
			}
    ...
    if (f.attributes) {
        ...
        for (var attr in f.attributes) {
			...
			if (attr!=image_attribute) {
                attr = attr.replace(/weight/g, '')
                attr = attr.replace(/score/g, '')
                attr = attr.replace(/links/g, 'co-assignees')
                attr = attr.replace(/</g, '')
                attr = attr.replace(/>/g, '')
